...
async function clearRows(page: Page, timeout: number): Promise<void> {
  ...
  throw new Error('BOP2-Eff-Tox: too many delete attempts while clearing rows.');
}
...
async function addRows(
  ...
  throw new Error(`BOP2-Eff-Tox: unable to add enough ${buttonName} rows (target=${target}).`);
}
...
async function fillInputs(inputs: Locator, values: string[], timeout: number): Promise<void> {
  for (let index = 0; index < values.length; index += 1) {
    ...
    if (actual !== values[index].trim()) {
      throw new Error(`BOP2-Eff-Tox: row ${index} read back "${actual}" instead of "${values[index]}".`);
    }
  }
}
...
export const configureInterimSampleSizes: KeywordHandler = async (page, ctx, step) => {
  ...
  if (values.length === 0) throw new Error('BOP2-Eff-Tox: no sample-size values found.');
